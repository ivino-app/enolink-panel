...
export default function RankingPageSimplified() {
    ...
    const { getRanking, getEventData, loading, error } = useRanking();
    ...
    async function loadWineRanking() {
        try {
            const rankingData = await getRanking(eventId);
            setWines(rankingData);

            // Calcular stats simples baseado nos dados retornados
            const averageRating = rankingData.length > 0 ? rankingData.reduce((sum, wine) => sum + wine.rating, 0) / rankingData.length : 0;

            // Para calcular participantes, seria necessário também retornar os ratings
            // Por enquanto, usando totalRatings como aproximação
            const totalRatings = rankingData.reduce((sum, wine) => sum + wine.totalRatings, 0);

            setStats({
                totalWines: rankingData.length,
                totalParticipants: totalRatings, // Aproximação - idealmente seria unique users
    ...
    const shareRanking = () => {
        if (navigator.share) {
            navigator.share({
                title: `Ranking - ${eventData?.name || "Evento"}`,
                text: "Confira o ranking dos vinhos deste evento!",
        ...
        return (
            <main className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-4">{error || "ID do evento não encontrado"}</p>
